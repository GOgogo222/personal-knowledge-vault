...
│   └── assets/        ← locally downloaded images attached to sources
...
3. Create a summary page in `wiki/sources/` named after the source slug.
4. Update `wiki/index.md` — add the new source page to the Sources section.
5. Update or create pages in `wiki/entities/` and `wiki/concepts/` for every significant entity or concept mentioned.
6. Update `wiki/overview.md` if the source changes or enriches the big picture.
...
4. Ask: "Should I save this answer as a wiki page?" If yes, create a page in `wiki/queries/` and update `wiki/index.md`.
...
- **Images**: If a source references images stored in `raw/assets/`, embed them with `![[assets/filename.png]]`. Read the text first; view images separately for additional context.
- **Do not modify `raw/`**: Treat all files in `raw/` as immutable. Never edit, move, or delete them.
